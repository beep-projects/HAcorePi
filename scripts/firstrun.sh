...
# For a full description see https://github.com/beep-projects/HAcorePi/readme.md
#
...
# for backward compatibility, a generic path to the correct folder should be used
BOOT_PATH=$(dirname "$(realpath -s "${BASH_SOURCE[0]}")")
echo "boot partition is mounted at ${BOOT_PATH}"

# copy some configuration variables into secondrun.sh 
#sed -i "s/^VARIABLE_NAME=.*/VARIABLE_NAME=${VARIABLE_NAME}/" ${bootPartition}/secondrun.sh
...
ConditionFileNotEmpty=${BOOT_PATH}/secondrun.sh

[Service]
WorkingDirectory=${BOOT_PATH}
ExecStart=${BOOT_PATH}/secondrun.sh
Type=oneshot
RemainAfterExit=no
...
#clean up
echo "removing firstrun.sh from ${BOOT_PATH}/cmdline.txt"
